...
| `mcp/` | **MCP (Model Context Protocol) client**: mount any MCP server's tools/resources/prompts as pi tools. Supports stdio & Streamable HTTP transports, auto-refresh on `list_changed` notifications. See [mcp.example.json](mcp/mcp.example.json). |
| `memory/` | **Long-term memory (Knowledge Graph)**: spawns the official `@modelcontextprotocol/server-memory` as a child process, auto-injects the full graph into system prompt before each turn, provides `memory__*` tools (`add_fact`, `search`, `read_graph`, `create_relations`, `forget_entity`). |
