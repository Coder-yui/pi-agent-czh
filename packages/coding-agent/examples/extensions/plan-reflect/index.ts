...
function getToolRetryKey(toolName: string, input: Record<string, unknown>): string {
...
export default function planReflectExtension(pi: ExtensionAPI): void {
	...
	function updateStatus(ctx: ExtensionContext): void {
		...
		ctx.ui.setStatus("plan-reflect", parts.length > 0 ? ctx.ui.theme.fg("muted", parts.join(" ")) : undefined);
	}
	...
	pi.registerCommand("plan", {
		...
		handler: async (args, ctx) => {
			...
			if (mode === "on" || mode === "off" || mode === "auto") {
				...
				ctx.ui.notify(`Planning mode: ${mode}`, "info");
			} else {
				ctx.ui.notify(`Current planning mode: ${state.config.plan}. Usage: /plan [auto|on|off]`, "info");
			}
			updateStatus(ctx);
			persist();
		},
	...
	pi.registerCommand("reflect", {
		...
		handler: async (args, ctx) => {
			...
			if (mode === "on" || mode === "off") {
				...
				ctx.ui.notify(`Reflection: ${mode}`, "info");
			} else {
				ctx.ui.notify(`Current reflection: ${state.config.reflect}. Usage: /reflect [on|off]`, "info");
			}
			updateStatus(ctx);
			persist();
		},
	...
	pi.on("before_agent_start", async (event) => {
		...
		// Retry limits apply to one user task only; never inherit failures from a prior prompt.
		state.retries = {};
...
Now begin with the plan.`;
	...
	/**
	 * tool_call: enforce the retry limit before the same failing action runs
	 * again. Reflection is useful guidance, but the guard must be deterministic.
	 */
	pi.on("tool_call", async (event) => {
		if (state.config.reflect !== "on") return;
		const target = getToolTarget(event.toolName, event.input);
		const record = state.retries[getToolRetryKey(event.toolName, event.input)];
		if (!record || record.attempts < state.config.maxRetries) return;
		state.stats.retriesBlocked++;
		persist();
		return {
			block: true,
			reason:
				`Retry limit reached for ${event.toolName} on "${target.slice(0, 80)}" after ` +
				`${record.attempts} failures. Inspect the surrounding context and use a different approach.`,
		};
	});
	...
	pi.on("tool_result", async (event, ctx) => {
		...
		const target = getToolTarget(event.toolName, event.input);
		const key = getToolRetryKey(event.toolName, event.input);
		...
		if (isMeaningfulError(event)) {
			...
			return {
				content: [{ type: "text" as const, text: reflectionNote }, ...event.content],
				isError: true,
			};
		}
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const saved = [...entries]
			.reverse()
			.find((e: { type?: string; customType?: string }) => e.type === "custom" && e.customType === "plan-reflect") as
			| { data?: PlanReflectState }
			| undefined;
		if (saved?.data) {
